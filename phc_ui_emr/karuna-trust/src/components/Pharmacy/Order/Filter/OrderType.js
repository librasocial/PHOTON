...
const OrderType = observer((props) => {
  const [checked, setChecked] = useState([]);
  let items1 = props.items1;
  let isChecked = (item) =>
    checked.includes(item) ? "checked-item" : "not-checked-item";

  const handleChecked = (e, label) => {
    if (e.target.checked == true) {
      items1.find((i) => i.label === label).checked = e.target.checked;
      if (label == "Purchase Order") {
        props.setSelectedType([...props.selectedType, "PO"]);
      } else {
        props.setSelectedType([...props.selectedType, label]);
      }
    } else {
      items1.find((i) => i.label === label).checked = e.target.checked;
      if (label == "Purchase Order") {
        props.setSelectedType(props.selectedType.filter((id) => id !== "PO"));
      } else {
        props.setSelectedType(props.selectedType.filter((id) => id !== label));
      }
    }
  };
  ...
  const handleSelectAll = () => {
    items1.map((i) => {
      props.selectedType.push(i.label);
    });
    items1.forEach((i) => {
      i.checked = true;
    });
