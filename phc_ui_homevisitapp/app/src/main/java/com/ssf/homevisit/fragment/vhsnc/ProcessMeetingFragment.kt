...
import com.ssf.homevisit.databinding.FragmentProcessMeetingBinding
...
class ProcessMeetingFragment : Fragment() {
    private lateinit var binding: FragmentProcessMeetingBinding
    private val viewModel: CreateMeetingViewModel by activityViewModels()
    var file: File? = null
    var imageUri: Uri? = null
    private val inviteSentDialog by lazy {
        val str =
            "VHSNC meeting documents have been saved and sent successfully!"
    ...
    private fun initializerComponents(
        ...
        binding = FragmentProcessMeetingBinding.inflate(inflater, container, false)
...
//        observeData()
    ...
    private fun initNewMeetingUI() {
        ...
        viewModel.uploadImageMap.forEach {
            binding.root.findViewWithTag<ImageUpload>(it.key)
                .setImageState(2, it.value.second, it.value.first)
        }
        ...
        binding.btnChangeVenue.setOnClickListener {
            val toEditVenue = ProcessMeetingFragmentDirections.processMeetingToReScheduleMeeting(
                requireArguments().getString("meetingId")!!, true
            )
            findNavController().navigate(toEditVenue)
        }
    }
    ...
    private fun setupBottomBar(enabled: Boolean) {
