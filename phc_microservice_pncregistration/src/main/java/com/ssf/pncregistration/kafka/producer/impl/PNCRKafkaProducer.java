package com.ssf.pncregistration.kafka.producer.impl;

import com.ssf.pncregistration.kafka.producer.PNCRProducer;
import com.ssf.pncregistration.kafka.producer.ProducerCallback;
 ...
 * topic
 */
public class PNCRKafkaProducer<K, V> implements PNCRProducer<K, V> {

    private final Logger logger = LoggerFactory.getLogger(PNCRKafkaProducer.class);

    private KafkaProducer<byte[], byte[]> producer;
    private boolean syncSend = true;
    private volatile boolean shutDown = false;

    public PNCRKafkaProducer(Properties producerConfig) {
        this(producerConfig, true);
    }

    public PNCRKafkaProducer(Properties producerConfig, boolean syncSend) {
