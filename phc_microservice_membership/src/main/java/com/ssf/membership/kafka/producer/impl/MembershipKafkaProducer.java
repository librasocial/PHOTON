package com.ssf.membership.kafka.producer.impl;

import java.util.Properties;
import java.util.concurrent.Future;

import com.ssf.membership.kafka.producer.MembershipProducer;
import com.ssf.membership.kafka.producer.ProducerCallback;
 ...
 *
 */
public class MembershipKafkaProducer<K, V> implements MembershipProducer<K, V> {

	private final Logger logger = LoggerFactory.getLogger(MembershipKafkaProducer.class);

	private KafkaProducer<byte[], byte[]> producer;
	private boolean syncSend = true;
	private volatile boolean shutDown = false;

	public MembershipKafkaProducer(Properties producerConfig) {
		this(producerConfig, true);
	}

	public MembershipKafkaProducer(Properties producerConfig, boolean syncSend) {
