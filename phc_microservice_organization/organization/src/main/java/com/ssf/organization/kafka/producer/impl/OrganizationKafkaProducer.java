package com.ssf.organization.kafka.producer.impl;



import com.ssf.organization.kafka.producer.OrganizationProducer;
import com.ssf.organization.kafka.producer.ProducerCallback;
 ...
 *
 */
public class OrganizationKafkaProducer<K, V> implements OrganizationProducer<K, V> {

	private final Logger logger = LoggerFactory.getLogger(OrganizationKafkaProducer.class);

	private KafkaProducer<byte[], byte[]> producer;
	private boolean syncSend = true;
	private volatile boolean shutDown = false;

	public OrganizationKafkaProducer(Properties producerConfig) {
		this(producerConfig, true);
	}

	public OrganizationKafkaProducer(Properties producerConfig, boolean syncSend) {
